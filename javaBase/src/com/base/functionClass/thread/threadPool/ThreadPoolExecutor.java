...
/**
 * 需要重新定义，workers中存储的，不是线程，是任务。 因为对于每一个 workers里面的存储的  Workder对象，并不是调用其start方法，使其到达就绪状态
 * 		算是 消费。  有的是直接执行其Runnable的实例变量的 run方法，算消费。所以集合中的 内容不能叫线程，而应该叫任务。
 * 		而clt 表示 线程池中  运行的 线程数，活跃线程数。
 * 执行流程
 * 	1. execute
 * 		若 若线程池中的 活跃线程数 < 核心线程数，
 * 			试着去 创建 一个新线程， addWork(runnable,true)--只要 小于核心线程数，就可以通过，继续向下去创建新线程
 * 				试着 将活跃线程数 +1 ，若不 成功，则重试  for循环判断，若成功，则判断当前线程池的状态，是否为running
 * 			如果创建成功，则
 * 
 * 		为了支持高并发，逻辑太绕了。
 * 			活跃线程数 < 核心线程数  -- 再次判断活跃线程线数小于核心线程数，（因为在此期间可能新建了线程，导致条件失效，若失效，则返回。走添加到队列的流程）
 * 				前面获取了 当前活跃线程数， 使用的是compareAndIncrementWorkerCount(c) ,他的意思是，如果等于期望值c，就 ctl+1，否则不加，返回false。返回false，则走 retry 。重新进入循环
 * 				这里有两种情况
 * 					1. 若 活跃线程数 还差很多，才到核心线程数，则每次循环只有一个线程 ，能够 compareAndIncrementWorkerCount，成功。然后继续向下走新建线程的流程。
 * 							其它线程，虽然都通过了前面的判断。但是compareAndIncrementWorkerCount 不成功，需要重新 走for循环尝试，然后又有一个线程 成功，走新建线程的流程。
 * 					2. 活跃线程数差一个 到达临界值。则，本次并发进入的所有线程，也是只有一个能够通过compareAndIncrementWorkerCount ，进入创建线程。其它的继续循环，然后 活跃线程数不小于 核心线程数。
 * 							创建线程的条件打破，走添加队列的流程。
 * 					**** 这里的   int c = ctl.get(); 得到的c相当与是一个 票据。不管哪个线程来，我都给你。但是， 这个票据 只能使用一次。不管哪个线程使用了。这个票据都失效了，需要重新去获取。
 * 							重新获取之后，可能影院的位置已经满了。只能 进入 队列等待执行。
 * 			活跃线程数 不小于 核心线程数
 * 				1. 将任务添加入队列。
 * 			
 * 			
...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    /**
     * 线程状态之间的转换
     * RUNNING -> SHUTDOWN ： 调用shutdown()， 重写了finalize()方法，里面调用了shutdown()
     * (RUNNING or SHUTDOWN) -> STOP : 调用shutdownNow() 方法
     * SHUTDOWN -> TIDYING  ： 当队列和线程池均为空的时候
     * STOP -> TIDYING ： 线程池为空的时候
     * TIDYING -> TERMINATED ： 处于TIDYING状态后最终会进入TERMINATED状态
     */
    ...
    private static int workerCountOf(int c)  { return c & CAPACITY; }// 获取线程池中未执行完的任务数，与capacity进行与操作，取低 29位k,capacity 全是1，所以是与。
    private static int ctlOf(int rs, int wc) { return rs | wc; } //rs是带后面0的，wc是带前面0的，进行或运行。
    ...
    private static boolean isRunning(int c) {//判断线程池状态  是否为正在运行 ，RUNNING 为负数。
        return c < SHUTDOWN;
    }
    ...
    private boolean compareAndDecrementWorkerCount(int expect) {//利用cas算法，-1
        return ctl.compareAndSet(expect, expect - 1);
    }
    ...
    private final HashSet<Worker> workers = new HashSet<Worker>();//用来存储线程池中的线程，线程都被封装成了Worker对象
...
//    	termination.signalAll(); //相当与线程的  notifyAll()
     ...
     */
    private long completedTaskCount;////记录了已经销毁的线程，完成的任务总数；
